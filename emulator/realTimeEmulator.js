...
.controller('PlatformCtrl',
    ["$sce", "$scope", "$rootScope", "$log", "$window", "$timeout", "emulatorMessageService",
      function ($sce, $scope, $rootScope, $log, $window, $timeout, emulatorMessageService) {
  'use strict';
  ...
  if (gameUrl === null) {
    ...
    console.log("You should pass the game url like this: ...platform.html?<GAME_URL> , e.g., http://yoav-zibin.github.io/emulator/emulator/realTimeEmulator.html?http://yoav-zibin.github.io/snake/index.html");
  }
  ...
  function sendMessageTo(index, msg) {
    ...
    $timeout(function () {
      emulatorMessageService.sendMessage(messageQueues[index].shift(), "game_iframe" + index);
  ...
  emulatorMessageService.addMessageListener(function (message, source) {
